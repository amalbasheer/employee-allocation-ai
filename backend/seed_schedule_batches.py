...
def seed_batches():
    ...
    def insert_batch(batch_name, domain, start, end, mentor_row, mode, session):
        with engine.begin() as conn:
            result = conn.execute(
                text("""
                    INSERT INTO student_batches
                    (batch_name, domain, start_date, end_date, mentor_id, status, delivery_mode, session)
                    VALUES (:name, :domain, :start, :end, :mentor_id, 'open', :mode, :session)
                    RETURNING batch_id
                """),
                {
    ...
    # --- DS: Suresh — April/May ---
    insert_batch("Apr DS Offline", "Data Science", date(2026, 4, 15), date(2026, 6, 14), suresh, "offline", "Tue & Thu - Morning")
    insert_batch("May DS Offline", "Data Science", date(2026, 5, 15), date(2026, 7, 14), suresh, "offline", "Tue & Thu - Evening")
    ...
    # --- DS: Anitha — June/July ---
    insert_batch("Jun DS Offline", "Data Science", date(2026, 6, 15), date(2026, 8, 14), anitha, "offline", "Mon & Wed - Morning")
    insert_batch("Jul DS Offline", "Data Science", date(2026, 7, 15), date(2026, 9, 14), anitha, "offline", "Tue & Thu - Afternoon")

    # --- DA: Nithya — April/July ---
    insert_batch("Apr DA Offline", "Data Analytics", date(2026, 4, 15), date(2026, 6, 14), nithya, "offline", "Mon & Wed - Morning")
    insert_batch("Jul DA Offline", "Data Analytics", date(2026, 7, 15), date(2026, 9, 14), nithya, "offline", "Tue & Thu - Afternoon")

    # --- DA: Aravind — May/June ---
    insert_batch("May DA Offline", "Data Analytics", date(2026, 5, 15), date(2026, 7, 14), aravind, "offline", "Tue & Thu - Morning")
    insert_batch("Jun DA Offline", "Data Analytics", date(2026, 6, 15), date(2026, 8, 14), aravind, "offline", "Mon & Wed - Afternoon")
