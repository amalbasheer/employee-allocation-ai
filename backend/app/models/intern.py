...
class InternsAndStudents(Base):
    ...
    intern_id = Column(String(20), primary_key=True, # Tells SQLAlchemy to let Postgres handle default generation
        server_default=text("'rp2-int-' || lpad(nextval('employee_id_seq')::text, 4, '0')"))
    ...
    __table_args__ = (
        ...
        CheckConstraint("intern_id ~ '^rp2-int-\\d{4}$'", name="check_employee_id_format"),
    )
